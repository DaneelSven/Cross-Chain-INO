 ...
 * @author Sven
 */
contract AdvancedNft is ERC721Enumerable, Ownable2Step, CommitReveal, ERC2771Context {
    ...
    modifier timedTransitions() {
        if (stage == Stages.preSale && block.timestamp >= creationTime + preSaleTime) {
            _nextStage();
        }
        if (stage == Stages.publicSale && block.timestamp >= creationTime + publicSaleTime) {
            _nextStage();
    ...
    event Claimed(address indexed claimant, uint256 index);
     ...
     */
    constructor(bytes32 $merkleRoot, address _trustedForwarder)
    ...
    function _contextSuffixLength() internal view override(Context, ERC2771Context) returns (uint256) {
        return 20; // or the appropriate length based on your implementation
    }
     ...
     */
    function whitelistMintBitmap(bytes32[] calldata _merkleProof, uint256 $index) public {
     ...
     */
    function MintNft() external timedTransitions atStages(Stages.preSale, Stages.publicSale) {
        if (!whitelistClaimed[msg.sender]) revert AlreadyClaimed();
     ...
     */
    function multiCallTransfer(address[] calldata $addresses, uint256[] calldata $tokenIds) external {
